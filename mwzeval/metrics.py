...
EVAL_ERROR_LOG_FNAME = "my_errorLog.txt"

def writeErrors(txt):
    with open(EVAL_ERROR_LOG_FNAME, "a") as f:
        f.write("\n"+"######"*20+"\n"+txt)
        ...
        for i, turn in enumerate(input_data[dialog_id]):
            ...
            for domain, gold_domain_state in gold_state.items():
                if domain not in turn["state"]:
                    turn_correct = False
                    writeErrors(f"[JGA] [{dialog_id}-{i}] domain not in turn[state]: {str(turn['state'])}, ||| ref {str(domain)}")
                    break
                for slot, value in gold_domain_state.items():
                    ...
                    if slot not in turn["state"][domain]:
                        writeErrors(f"[JGA] [{dialog_id}-{i}] slot not in turn[state][domain]: {str(turn['state'][domain])}, ||| (gt:) {str(slot)}")
                        turn_correct = False
                        break
                    value = value.lower()
                    pred_value = turn["state"][domain][slot].lower()
                    if value != pred_value:
                        writeErrors(f"[JGA] [{dialog_id}-{i}] value != pred_value. For slot: {slot} in domain {domain} (predicted:) {str(turn['state'][domain][slot].lower())}, ||| (gt:) {str(value)}")
                        turn_correct = False
                        break
                if not turn_correct:
                    break
...
def get_dialog_success(goal, booked_domains, utterances, states, domain_estimates, database, dialog_id=""):
    ...
    for domain in goal:
        ...
        if offered_venues[domain] == 'MATCHED':
            ...
            if set(offered_venues[domain]).issubset(set(goal_venues)):
                ...
                writeErrors(f"{dialog_id}-{t_idx} [MATCH] domain: {domain} ||| (predicted:) offered_venues[domain]: {set(offered_venues[domain])} ||| (predicted)  state[domain] { state[domain]} ||| (gt:) state: {goal[domain]['informable']} ||| (gt:) set(goal_venues): {set(goal_venues)} ||| (predicted:) system_utterance: {system_utterance} ||| (gt:) goal: {str(goal)}")
        else:
            try:
                writeErrors(f"{dialog_id}-{t_idx}[MATCH] domain: {domain} ||| (predicted:) offered_venues[domain]: {set(offered_venues[domain])} ||| (predicted:) system_utterance: {system_utterance} ||| (predicted)  state[domain] {state[domain]}  ||| (gt:) state: {goal[domain]['informable']} ||| (gt:) goal: {str(goal)}")
            except:
                writeErrors(f"Error while trying to log {dialog_id}-{t_idx}[MATCH] domain: {domain}")
    ...
    if match['total']:
        for domain in goal:
            ...
            if  len(provided_and_wanted_slots) != len(requestable_slots_in_goal[domain]):
                writeErrors(f"{dialog_id}-{t_idx} [SUCCESS] domain: {domain} ||| (predicted:) provided_requestable[domain]: {provided_requestable_slots[domain]} ||| (gt:) requestable_slots_in_goal[domain]: {requestable_slots_in_goal[domain]} ||| (predicted:) system_utterance: {system_utterance} ||| (gt:) goal: {str(goal)}")
            success[domain] = domain_success
            
        success['total'] = (sum(success.values()) >= len(success.keys()))
    else:
        writeErrors(f"{dialog_id}-{t_idx} [SUCCESS] match[total] is false. match.values: {str(match.values())} ||| match.keys: {str(match.keys())}" )
...
def get_dst(input_data, reference_states, fuzzy_ratio=95):
    ...
    def compare(hyp, ref):
        ...
        # lower all slot values
        for slot, value in hyp.items():
            hyp[slot] = value.lower()
    ...
    if not has_state_predictions(input_data):
        ...
        for dialog_id in input_data:
            for i, turn in enumerate(input_data[dialog_id]):
                ...
                if tp != len(hyp):
                    writeErrors(f"[DST] [{dialog_id}-{i}] hyp: {str(hyp)}, ||| ref {str(ref)}")
